...
async function exportarCensoExcelBase(req, res, rows, { desde, hasta, titulo, nombreArchivo }) {
...
async function censoMensual(req, res) {
  ...
  try {
    const rows = await obtenerRowsCensoGeneral(desde, hasta);
...
// ============================================================
// GET /api/reportes/censo/primer-control?desde=YYYY-MM-DD&hasta=YYYY-MM-DD
// ============================================================
async function censoMensualPrimerControl(req, res) {
  const { desde, hasta } = req.query;
  ...
  try {
    const rows = await obtenerRowsCensoPrimerControl(desde, hasta);
    return res.json({
      desde,
      hasta,
      total: rows.length,
    ...
    console.error(err);
    return res.status(500).json({ error: 'Error al generar censo de primer control' });
  }
}
...
async function exportarCensoExcel(req, res) {
  ...
  try {
    const rows = await obtenerRowsCensoGeneral(desde, hasta);
    await exportarCensoExcelBase(req, res, rows, {
      desde,
      hasta,
      titulo: 'CENSO NOMINAL DE MUJERES EMBARAZADAS',
      nombreArchivo: `censo_${desde}_${hasta}.xlsx`,
    });
  } catch (err) {
    console.error(err);
...
// ============================================================
// GET /api/reportes/censo/primer-control/excel?desde=...&hasta=...
// ============================================================
async function exportarCensoPrimerControlExcel(req, res) {
  const { desde, hasta } = req.query;
  ...
  try {
    ...
    await exportarCensoExcelBase(req, res, rows, {
      ...
      nombreArchivo: `censo_primer_control_${desde}_${hasta}.xlsx`,
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Error exportando Excel de primer control' });
  }
}
...
// ============================================================
// GET /api/reportes/proximas-a-parir
// Pacientes con FPP en los próximos 30 días
// ============================================================
async function proximasAParir(req, res) {
  try {
    const { rows } = await pool.query(`
      SELECT
        p.id,
    ...
    console.error(err);
    return res.status(500).json({ error: 'Error al obtener pacientes próximas a parir' });
  }
}
 
// ============================================================
// GET /api/reportes/sin-control-reciente
// Pacientes sin controles en las últimas 4 semanas
// (solo las que tienen FPP futura — aún embarazadas)
// ============================================================
async function sinControlReciente(req, res) {
  try {
    const { rows } = await pool.query(`
      SELECT
        p.id,
    ...
    console.error(err);
    return res.status(500).json({ error: 'Error al obtener pacientes sin control reciente' });
  }
}
