...
function tdapAuditEvent(result, {
  event = 'preparar',
  reason = result.dispatch?.status || 'consulta_completada',
} = {}) {
  return {
    contexto: {
      categoria: 'automatizaciones',
      entidad: 'seguimiento_tdap_el_chal',
      evento: event,
    },
    accion: event === 'consultar' ? 'consultar' : 'actualizar',
    metadata: {
      tipo_automatizacion: 'seguimiento_tdap_el_chal',
      resultado: 'exitoso',
      motivo_codigo: reason,
      cantidad_nuevas: result.new_opportunities?.total ?? result.newTotal ?? 0,
      cantidad_pendientes: result.pending?.total ?? result.pendingTotal ?? 0,
      fecha_desde: result.range?.from,
      fecha_hasta: result.range?.to,
    },
  };
...
function tdapFailureEvent(event, reason) {
  return {
    contexto: {
      categoria: 'automatizaciones',
      entidad: 'seguimiento_tdap_el_chal',
...
function createAutomatizacionesController({
  ...
  async function prepararTdap(req, res, next) {
    try {
      const result = await service.prepararDespachoTdap();
      await bestEffortAudit(audit, tdapAuditEvent(result));
      res.set({ 'Cache-Control': 'no-store', Pragma: 'no-cache' });
      return res.json(result);
    } catch (error) {
      await bestEffortAudit(audit, tdapFailureEvent(
        'preparar',
        error.code || 'preparacion_interna_fallida'
      ));
      if (error instanceof AppError) return next(error);
      return next(new AppError(
        500,
        'No se pudo preparar el seguimiento Tdap',
        { code: 'AUTOMATION_INTERNAL_ERROR' }
      ));
    }
  }

  async function descargarTdapExcel(req, res, next) {
    try {
      const result = await service.generarSeguimientoTdapExcel({
        dispatchToken: req.automationDispatchToken,
      });
      await bestEffortAudit(audit, tdapAuditEvent(result, {
        event: 'consultar',
        reason: 'excel_generado',
      }));
      res.set({
        'Cache-Control': 'private, no-store, max-age=0',
        Pragma: 'no-cache',
        ...
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="${result.filename}"`,
        'X-CAP-New-Count': String(result.newTotal),
        'X-CAP-Pending-Count': String(result.pendingTotal),
      });
      return res.send(result.content);
    } catch (error) {
      await bestEffortAudit(audit, tdapFailureEvent(
        'consultar',
        error.code || 'excel_interno_fallido'
      ));
      if (error instanceof AppError) return next(error);
      return next(new AppError(
        500,
        'No se pudo generar el Excel de seguimiento Tdap',
        { code: 'AUTOMATION_INTERNAL_ERROR' }
      ));
    }
  }

  async function confirmarTdap(req, res, next) {
    try {
      const result = await service.confirmarDespachoTdap({
        dispatchToken: req.automationDispatchConfirmation.dispatch_token,
      });
      await bestEffortAudit(audit, tdapAuditEvent(result, {
        event: 'confirmar',
      ...
      return next(new AppError(
        500,
        'No se pudo confirmar el despacho Tdap',
        { code: 'AUTOMATION_INTERNAL_ERROR' }
      ));
    }
  }

  async function resolverTdap(req, res, next) {
    try {
      const body = req.automationDispatchResolution;
      const result = await service.resolverDespachoTdap({
        desde: body.desde,
        hasta: body.hasta,
        resolucion: body.resolucion,
        motivoCodigo: body.motivo_codigo,
      ...
      return next(new AppError(
        500,
        'No se pudo resolver el despacho Tdap',
        { code: 'AUTOMATION_INTERNAL_ERROR' }
      ));
    }
  }
