...
isolatedTest('migracion 008 en cuatro bases PostgreSQL temporales', async (t) => {
  ...
  try {
    ...
    await t.test('A: tabla vacia se elimina y el runner es idempotente', async () => {
      ...
      assert.match(first.entries.log[0], /14 aplicada\(s\), 0 omitida\(s\)/);
      ...
      await withClient(url, async (client) => {
        ...
        assert.equal((await publicTables(client)).length, 20);
      });

      const second = await runMigration(url);
      assert.equal(second.ok, true);
      assert.match(second.entries.log[0], /0 aplicada\(s\), 14 omitida\(s\)/);
      await withClient(url, async (client) => {
        const { rows } = await client.query(
    ...
    await t.test('C: instalacion limpia registra 008 y conserva 19 tablas finales', async () => {
